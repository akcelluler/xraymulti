...
NUMBER_OF_CLIENTS=$(grep -c -E "^##### " "/etc/xray/configmultiakun.json")
	...
	echo -e	"  NO ${GREEN}USER   ${RED}EXPIRED ${BLUE}Net${NC}"
        grep -E "^##### " "/etc/xray/configmultiakun.json" | cut -d ' ' -f 2-3 | nl -s ') '
...
#buatlinktrojan
trojanlinkwstls="trojan://${uuid}@${bugws}:443?path=/xraytrojanws&security=tls&host=${domain}&type=ws&sni=${domain}#${user}"
trojanlinkgrpc="trojan://${uuid}@${bugws}:443?mode=gun&security=tls&type=grpc&serviceName=trojan-grpc&sni=${domain}#${user}"
...
#buatlinkvmess
cat>/etc/xray/configws-vmess-$user-ws.json<<EOF
      {
      "v": "2",
      "ps": "${user}",
      "add": "${bugws}",
...
EOF
cat>/etc/xray/configws-vmess-$user-wstls.json<<EOF
      {
      "v": "2",
      "ps": "${user}",
      "add": "${bugws}",
...
EOF
cat>/etc/xray/configws-vmess-$user-grpc.json<<EOF
      {
      "v": "2",
      "ps": "${user}",
      "add": "${bugws}",
...
shadowsockslink="ss://${shadowsocks_base64e}@$bugws:$tls?plugin=xray-plugin;mux=0;path=/xrayssws;host=$domain;tls#${user}"
shadowsockslinkgrpc="ss://${shadowsocks_base64e}@$bugws:$tls?plugin=xray-plugin;mux=0;serviceName=ss-grpc;host=$domain;tls#${user}"
#buatlinkconfigtextshadowshock
cat > /home/vps/public_html/configws-ss-ws-$user.txt<<EOF
{ 
 "dns": {
    "servers": [
      "8.8.8.8",
...
EOF
cat > /home/vps/public_html/configws-ss-grpc-$user.txt<<EOF
{
    "dns": {
    "servers": [
      "8.8.8.8",
...
clear
#buat text all link
cat > /home/vps/public_html/configws-multiakun-$user.txt<<EOF
${trojanlinkwstls}
${trojanlinkgrpc}
${vlesslinkws}
${vlesslinkwstls}
...
echo -e "URL Custom Config WS TLS   : ${NC}${ORANGE} http://${domain}:89/configws-ss-ws-$user.txt ${NC}" 
echo -e "URL Custom Config GRPC TLS : ${NC}${BLUE} http://${domain}:89/configws-ss-grpc-$user.txt ${NC}" 
echo -e "===================================================="
echo -e "======Custom Import Config From URL ======="
echo -e "URL CONFIG V2RAYNG all trojan-vless-vmess : ${NC}${RED} http://${domain}:89/configws-multiakun-$user.txt ${NC}" 
echo -e "===================================================="
